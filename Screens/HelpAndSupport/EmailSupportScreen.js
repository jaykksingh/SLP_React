...
const EmailSupportScreen = ({route,navigation})  => {
	...
	let contactArray = supportDetails.email.length > 0 ? supportDetails.email[0].default : []
	return(
		<SafeAreaView style={styles.container}>			
			<FlatList style={{}}
				...
				renderItem={({item, index}) => 
					<View>
						{
						item.label == 'General Queries' ? 
						<View style={{alignItems:'center',paddingRight:8, paddingTop:24, paddingBottom:24}} >
							<Text style={{fontSize:18, color:ThemeColor.TextColor,fontFamily: FontName.Regular}}>{item.value}</Text>
							<Text style={{flex:1, fontSize:14, color:ThemeColor.SubTextColor,fontFamily: FontName.Regular,marginTop:8}}>General Support</Text>
						</View> : 
