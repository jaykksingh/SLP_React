...
const ApprovedTimesheetScreen = ({navigation})  => {
  ...
  const  getTimeshetsList = async(startDate, endDate) => {
    ...
    console.log(startDate,endDate);
		...
		axios ({  
		  ...
		  data:{"status":"approved",'startDate':startDate,'endDate':endDate}
		})
		.then((response) => {
  ...
  const handleTSRequestChange = (item) =>{
    let message = `Please unlock my timesheet for the week - ${getFormatedDateRange(item)}`;
      ...
      {transformedArray.length > 0 ?
       <SectionList 
          ...
          renderItem={({item, index,section}) => 
            <View style={{ backgroundColor:'#fff'}}>
              <TouchableOpacity style={{}} onPress={()=> navigation.navigate('ViewTimesheet',{timesheetDetails:item,projectDetail:section})}>
                <View style={{height:40,paddingLeft:16, flexDirection:'row', alignItems:'center',}}>
                ...
                <View style={{height:40,paddingLeft:16, flexDirection:'row', alignItems:'center',flex: 1}}>
                  <Text style={{fontSize:14, color:ThemeColor.GreenColor,fontFamily: FontName.Regular, flex: 1, textAlign: 'center'}}>Approved</Text>
                  <View style={{height:40 , width:1,backgroundColor:ThemeColor.BorderColor}}/>
                  <TouchableOpacity style={{flex: 1,alignItems:'center',justifyContent: 'center', flexDirection:'row'}} onPress = {() => {handleTSRequestChange(item)}}>
                    <Feather name="lock" color={ThemeColor.NavColor} size={18,18} />
                    <Text style={{fontSize:14, color:ThemeColor.NavColor,fontFamily: FontName.Regular, textAlign: 'center', marginLeft:8}}>Request changes</Text>
                  </TouchableOpacity>
...
export default ApprovedTimesheetScreen;
